#!/bin/bash
# Sync
telegram -M "OrangeFox (R10.1) - A20s: Sync started"
SYNC_START=$(date +"%s")
...
repo sync
git clone https://github.com/yukosky/android_device_samsung_a207M.git -b android-9.0-R9 device/samsung/a207M
mv device/samsung/a207M/buildfox.sh .
chmod +x buildfox.sh

SYNC_END=$(date +"%s")
SYNC_DIFF=$((SYNC_END - SYNC_START))
telegram -M "OrangeFox (R10.1) - A20s: Sync completed successfully in $((SYNC_DIFF / 60)) minute(s) and $((SYNC_DIFF % 60)) seconds"
